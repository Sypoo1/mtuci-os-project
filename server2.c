...
bool ensure_single_instance(void) {
    ...
    lock_fd = check(open("/tmp/my_server2.lock", O_CREAT | O_RDWR, 0666),
                    "open lockfile");
...
void *handle_connection(void *p_client_socket) {
    ...
    // Сразу отправляем OK с текущим временем
    {
        ...
        int len =
            snprintf(reply, sizeof(reply), "HELLO FROM SERVER 2 %s\n", ts);
